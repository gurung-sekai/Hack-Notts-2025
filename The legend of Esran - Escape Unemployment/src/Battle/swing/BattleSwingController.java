...
public class BattleSwingController {
    ...
    /** Player chooses index 0..3. Resolves a full round and returns a readable log. */
    public String playerTurn(int playerMoveIndex) {
        if (over) return "The battle is already over.";
        playerMoveIndex = Math.max(0, Math.min(3, playerMoveIndex));

        Technique tA = BaseMoves.MOVES[playerMoveIndex];
        if (a.cd.containsKey(tA) && a.cd.get(tA) > 0) return "That move is on cooldown.";
        ...
        if (a.isDown() || b.isDown()) { over = true; log.append("Winner: ").append(getWinnerName()); }
        return log.toString();
    }

    private static class Act { Fighter u,t; Technique k; Act(Fighter u,Fighter t,Technique k){this.u=u;this.t=t;this.k=k;}}

    private String resolve(Act act) {
        Fighter u=act.u, t=act.t; Technique tech=act.k;
    ...
    private void setCooldown(Fighter f, Technique t){ if (t.cooldown > 0) f.cd.put(t, t.cooldown); }
}
