...
public class BaseMoves {
    ...
    public static final Technique FLAME_LASH = new Technique(
            ...
            (u, t) -> {
                if (Rng.roll(30) && t.status == Status.OK) {
                    t.status = Status.IGNITED;
                    System.out.println(t.name + " is ignited!");
                }
            });
    ...
    public static final Technique THORN_BIND = new Technique(
            ...
            (u, t) -> {
                if (Rng.roll(50)) {
                    t.status = Status.ROOTED;
                    System.out.println(t.name + " is rooted in place!");
                }
            });
    ...
    public static final Technique BRACE = new Technique(
            ...
            (u, t) -> {
                u.base.guard += 2;
                System.out.println(u.name + " braces (+Guard)!");
            });

    public static final Technique DISRUPT_BOLT = new Technique(
            "Disrupt Bolt", Affinity.STORM, 18, 100, 3, 1, 2, Tag.INTERRUPT,
            (u, t) -> {
                if (Rng.roll(35) && t.status == Status.OK) {
                    t.status = Status.SHOCKED;
                    System.out.println(t.name + " is shocked!");
                }
            });

    // The universal moveset
    public static final Technique[] MOVES = {
