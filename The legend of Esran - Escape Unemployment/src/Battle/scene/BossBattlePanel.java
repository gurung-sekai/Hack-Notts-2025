...
public class BossBattlePanel extends JPanel {
    ...
    private void chooseCommand() {
        BattleEngine.RoundOutcome outcome = engine.resolve(cmdIndex);
    ...
    private void applyOutcome(BattleEngine.RoundOutcome outcome) {
        addMessages(outcome.messages);

        for (BattleEngine.Event event : outcome.events) {
            switch (event.type) {
                case DAMAGE -> {
    ...
    private void spawnEffect(BattleEngine.Event event) {
        FighterVisual actor = visualFor(event.actor);
        FighterVisual target = visualFor(event.target);
